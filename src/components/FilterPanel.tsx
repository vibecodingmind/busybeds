    ...
    <>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full border text-sm font-medium transition-all whitespace-nowrap ${
              params.sort && params.sort !== 'best'
                ...
                : 'border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-200 hover:border-gray-400 dark:hover:border-slate-400'
            }`}
          >
          ...
          {showSort && (
            <>
              ...
              <div className="absolute right-0 top-11 z-50 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-2xl py-2 min-w-[200px]">
                {SORT_OPTIONS.map(o => {
                  const active = (params.sort || 'best') === o.value;
                  return (
                    <button key={o.value} onClick={() => applySort(o.value)}
                      className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors flex items-center gap-2.5 ${active ? 'font-semibold text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-300'}`}>
                      {active
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full border text-sm font-medium transition-all whitespace-nowrap ${
            activeFilterCount > 0
              ? 'border-gray-900 dark:border-gray-100 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900'
              : 'border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 bg-white dark:bg-slate-800 hover:border-gray-400 dark:hover:border-slate-400'
          }`}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
          ...
          Filters
          {activeFilterCount > 0 && (
            <span className="w-5 h-5 rounded-full bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-xs font-bold flex items-center justify-center leading-none">
              {activeFilterCount}
            </span>
          )}
        </button>
      ...
      {showFilters && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">

            {/* Modal header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-700 flex-shrink-0">
              <button onClick={resetFilters} className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 underline transition-colors">
                Reset all
              </button>
              <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">Filters</h3>
              <button onClick={() => setShowFilters(false)}
                className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 transition-colors">
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-6">
                  ...
                  {SORT_OPTIONS.map(o => (
                    <button key={o.value} onClick={() => setSort(o.value)}
                      className={`px-3 py-2.5 rounded-xl border text-sm font-medium transition-all text-left ${
                        sort === o.value ? 'border-gray-900 dark:border-gray-100 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900' : 'border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 hover:border-gray-400 dark:hover:border-slate-400'
                      }`}>
                      {o.label}
                    </button>
                  ))}
              ...
              {/* Star rating */}
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-3">Star rating</h4>
                <div className="flex gap-2 flex-wrap">
                  {[1,2,3,4,5].map(n => (
                    <button key={n} onClick={() => toggleStar(String(n))}
                      className={`flex items-center gap-1.5 px-4 py-2 rounded-full border text-sm font-medium transition-all ${
                        stars.includes(String(n)) ? 'border-gray-900 dark:border-gray-100 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900' : 'border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 hover:border-gray-400 dark:hover:border-slate-400'
                      }`}>
              ...
              {/* Price range */}
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-3">Price per night</h4>
                <div className="flex items-center gap-3">
                  <div className="flex-1 relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none">$</span>
                    <input type="number" placeholder="Min" value={minPrice} onChange={e => setMinPrice(e.target.value)}
                      className="w-full pl-7 pr-3 py-2.5 border border-gray-200 dark:border-slate-600 rounded-xl text-sm bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-gray-900 dark:focus:border-gray-400 transition-colors" />
                  </div>
                  <span className="text-gray-400 dark:text-gray-500 flex-shrink-0 font-medium">—</span>
                  <div className="flex-1 relative">
              ...
              {/* Minimum discount */}
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-3">Minimum discount</h4>
                <div className="flex gap-2 flex-wrap">
                  {['10','20','30','50'].map(d => (
                    <button key={d} onClick={() => setMinDiscount(minDiscount === d ? '' : d)}
                      className={`px-4 py-2 rounded-full border text-sm font-medium transition-all ${
                        minDiscount === d ? 'border-gray-900 dark:border-gray-100 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900' : 'border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 hover:border-gray-400 dark:hover:border-slate-400'
                      }`}>
                      {d}%+
                    </button>
                  ))}
              ...
              {/* Amenities */}
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-3">Amenities</h4>
                <div className="grid grid-cols-2 gap-2">
                    ...
                    return (
                      <button key={a} onClick={() => toggleAmenity(a)}
                        className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm text-left transition-all ${
                          on ? 'border-gray-900 dark:border-gray-100 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900' : 'border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 hover:border-gray-400 dark:hover:border-slate-400'
                        }`}>
              ...
              {/* Vibe Tags */}
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-3">Vibe</h4>
            ...
            {/* Sticky footer */}
            <div className="px-6 py-4 border-t border-gray-100 dark:border-slate-700 flex-shrink-0 bg-white dark:bg-slate-900">
